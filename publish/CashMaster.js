  ...
  */
  ...
  on('chat:message', function (msg) {
    ...
    if (msg.content.includes("--help") || msg.content == "!cm") {
      sendChat(scname, "/w gm <h1 id='cashmaster'>CashMaster</h1><p>A currency management script for the D&amp;D 5e OGL and 5e Shaped sheets on Roll20.net.</p><p>Please use <code>!cm</code> for inline help and examples.</p><h2 id='setup'>Setup</h2><p>Make sure you use the correct sheet setting (<code>OGL</code> or <code>5E-Shaped</code>).</p><h2 id='usage'>Usage</h2><p>First, select one or several party members. </p><p>Then use </p><ul><li><code>!cm --overview</code> to get an <strong>overview</strong> over the party&#39;s cash, </li><li><code>!cm --share</code> to <strong>convert and share</strong> the money equally between party members, converting the amount into the best combination of gold, silver and copper (this should be used in smaller stores),</li><li><code>!cm --convert</code> to <strong>convert and share</strong> the money equally between party members, converting the amount into the best combination of platinum, gold, electrum, silver and copper (this should only be used in larger stores that have a fair amount of cash),</li><li><code>!cm --add [amount][currency]</code> to <strong>add</strong> an equal amount of money to each selected party member,</li><li><code>!cm --hoard [amount][currency]</code> to <strong>split up</strong> a certain amount of coins between the party members, like a found treasure. Note that in this case, no conversion between the different coin types is made - if a party of 5 shares 4 pp, then 4 party members receive one pp each, and the last member won&#39;t get anything.</li><li><code>!cm --pay [amount][currency]</code> to let each selected party member <strong>pay</strong> a certain amount. The script will even try to take higher and lower coin types to get the full amount. E.g. to pay 1gp when the character has no gold, the script will use 1pp (and return 9gp), or it will take 2ep, 10sp or 100cp - or any other valid combination of coins - to pay the desired amount.</li></ul><p><strong>Note:</strong> You can use several coin values at once, e.g. <code>!cm --hoard 50gp 150sp 2000cp</code> or <code>!cm --pay 2sp 5cp</code>.</p><h3 id='examples'>Examples</h3><ol><li><code>!cm --overview</code> will show a cash overview.</li><li><code>!cm --share</code> will collect all the money and share it evenly on the members, using gp, sp and cp only (pp and ep will be converted). Can also be used for one character to &#39;exchange&#39; money.</li><li><code>!cm --convert</code> - same as <code>!cm --share</code>, but will also use platinum and electrum.</li><li><code>!cm --add 50gp</code> will add 50 gp to every selected character.</li><li><code>!cm --hoard 50gp</code> will (more or less evenly) distribute 50 gp among the party members.</li><li><code>!cm --pay 10gp</code> will subtract 10gp from each selected character. It will try to exchange the other coin types (e.g. it will use 1pp if the player doesn&#39;t have 10gp).</li></ol><h2 id='credits'>Credits</h2><p>With thanks to mlenser for his contributions.</p>");
